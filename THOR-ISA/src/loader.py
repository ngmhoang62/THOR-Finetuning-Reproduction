...
class MyDataLoader:
    ...
    def get_data(self):
        ...
        train_loader = DataLoader(MyDataset(train_data), num_workers=0, worker_init_fn=self.worker_init, \
                                  shuffle=self.config.shuffle, batch_size=self.config.batch_size,
                                  collate_fn=self.collate_fn)
        # Fix validation/test shuffle bug (always set shuffle=False)
        valid_loader = DataLoader(MyDataset(valid_data), num_workers=0, worker_init_fn=self.worker_init, \
                                  shuffle=False, batch_size=self.config.batch_size,
                                  collate_fn=self.collate_fn)
        test_loader = DataLoader(MyDataset(test_data), num_workers=0, worker_init_fn=self.worker_init, \
                                 shuffle=False, batch_size=self.config.batch_size,
                                 collate_fn=self.collate_fn)
    ...
    def _apply_chat_template(self, system_msg, user_msg):
        # Specific handler to utilize Gemma's instruction/chat format if gemma is used
        if 'gemma' in self.config.model_path.lower():
    ...
    def collate_fn(self, data):
        ...
        # Save raw lists to match predictions with raw records for exporting to CSV
        raw_texts = list(input_tokens)
        raw_targets = list(input_targets)
        ...
        if self.config.reasoning == 'prompt':
            ...
            for i, line in enumerate(input_tokens):
                ...
                # Apply chat template if using chat/instruct model
                prompt = self._apply_chat_template("You are an expert of sentiment and opinion analysis.", prompt)

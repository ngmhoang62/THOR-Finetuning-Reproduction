...
class PromptTrainer:
    ...
    def train_step(self):
        ...
        for i, data in enumerate(train_data):
            # Strip metadata strings that are used for CSV log extraction
            model_data = {k: v for k, v in data.items() if k not in ['raw_texts', 'raw_targets']}
            loss = self.model(**model_data)
    ...
    def evaluate_step(self, dataLoader=None, mode='valid'):
        ...
        # Track predictions for CSV export
        all_raw_texts = []
        all_raw_targets = []
        all_implicits = []
        all_gold_labels = []
        ...
        for i, data in tqdm(enumerate(dataiter), total=dataLoader.data_length):
            with torch.no_grad():
                model_data = {k: v for k, v in data.items() if k not in ['raw_texts', 'raw_targets']}
                output = self.model.evaluate(**model_data)
                self.add_output(data, output)
                
                # Append elements for CSV
                all_raw_texts.extend(data['raw_texts'])
                all_raw_targets.extend(data['raw_targets'])
                all_implicits.extend(data['implicits'].tolist())
                all_gold_labels.extend(data['input_labels'].tolist())
                all_predictions.extend(output)
                
        result = self.report_score(mode=mode)
        
        # Save detailed logs to CSV if we are evaluating the test set
        if mode == 'test':
            os.makedirs('results', exist_ok=True)
            df = pd.DataFrame({
                'text': all_raw_texts,
                'target': all_raw_targets,
                'implicit': all_implicits,
                'gold_label': [self.config.label_list[g] for g in all_gold_labels],
                'predicted_label': [self.config.label_list[p] for p in all_predictions]
            })
            model_slug = self.config.model_path.replace('/', '-').replace('\\', '-')
            df.to_csv(f'results/predictions_prompt_{self.config.data_name}_{model_slug}.csv', index=False)
            print(f"Predictions saved to results/predictions_prompt_{self.config.data_name}_{model_slug}.csv")
...
class ThorTrainer:
    ...
    def _apply_majority_vote(self, seqs):
        # Apply voting over num_return_sequences if self_consistency is enabled
        if hasattr(self.config, 'self_consistency') and self.config.self_consistency:
            n = self.config.self_consistency_n
            voted = []
            for idx in range(0, len(seqs), n):
                sub_group = seqs[idx:idx+n]
                most_common = Counter(sub_group).most_common(1)[0][0]
    ...
    def prepare_step_two(self, aspect_exprs, data):
        ...
        for context, target, aspect_expr in zip(contexts_A, targets, aspect_exprs):
            ...
            # Apply instruct template
            prompt = self.model._apply_chat_template("You are an expert of sentiment and opinion analysis.", prompt)
    ...
    def train_step(self):
        ...
        for i, data in enumerate(train_data):
            model_data = {k: v for k, v in data.items() if k not in ['raw_texts', 'raw_targets']}
            step_one_inferred_output = self.model.generate(**model_data)

            # Support Self-Consistency logic during training if active
            step_one_inferred_output = self._apply_majority_vote(step_one_inferred_output)

            step_one_inferred_data = self.prepare_step_two(step_one_inferred_output, model_data)
            step_two_inferred_output = self.model.generate(**step_one_inferred_data)
            step_two_inferred_output = self._apply_majority_vote(step_two_inferred_output)
            ...
            step_three_inferred_output = self._apply_majority_vote(step_three_inferred_output)

            step_label_data = self.prepare_step_label(step_three_inferred_output, step_two_inferred_data, model_data)
    ...
    def evaluate_step(self, dataLoader=None, mode='valid'):
        ...
        # Log variables for CSV prediction export
        all_raw_texts = []
        all_raw_targets = []
        all_implicits = []
        all_gold_labels = []
        ...
        if mode == 'test':
            ...
            model_slug = self.config.model_path.replace('/', '-').replace('\\', '-')
            df.to_csv(f'results/predictions_thor_{self.config.data_name}_{model_slug}.csv', index=False)
            print(f"Predictions saved to results/predictions_thor_{self.config.data_name}_{model_slug}.csv")
